...
Source2		: %{name}-logrotate.in
Source3		: %{name}-java-gateway.service
Source4		: %{name}-agent.service
Source5		: %{name}-server.service
Source6		: %{name}-proxy.service
Patch0		: config.patch
Patch1		: fonts-config.patch
Patch100	: %{name}-foreground.patch
...
# init scripts
install -m 0644 -p %{SOURCE3} $RPM_BUILD_ROOT%{_unitdir}/zabbix-java-gateway.service
install -m 0644 -p %{SOURCE4} $RPM_BUILD_ROOT%{_unitdir}/zabbix-agent.service
install -m 0644 -p %{SOURCE5} $RPM_BUILD_ROOT%{_unitdir}/zabbix-server.service
install -m 0644 -p %{SOURCE6} $RPM_BUILD_ROOT%{_unitdir}/zabbix-proxy.service
...
%changelog
* Sun Jul 20 2014 makocchi <makocchi@gmail.com> - 2.2.5-1
